...
export default function HomePage() {
  ...
  // Local auth options component (Google or Email/Password)
  function AuthOptions({ redirectToLogin, navigate }: { redirectToLogin: () => void; navigate: ReturnType<typeof useNavigate> }) {
    const [authView, setAuthView] = useState<'google' | 'email'>('google');
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-center gap-2 mb-2">
          <button
            ...
            className={`px-3 py-2 rounded-lg text-sm ${authView === 'google' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
          >
            Sign in with Google
          </button>
          <button
            onClick={() => setAuthView('email')}
            className={`px-3 py-2 rounded-lg text-sm ${authView === 'email' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
          >
            Email / Password
          </button>
        </div>
        ...
        {authView === 'google' ? (
          ...
          <form onSubmit={handleEmailSubmit} className="space-y-4">
            {authError && (
              <div className="p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-800">{authError}</div>
            )}

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Email</label>
              <input type="email" value={formEmail} onChange={(e) => setFormEmail(e.target.value)} required className="w-full px-4 py-2 border rounded-lg" />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Password</label>
              <input type="password" value={formPassword} onChange={(e) => setFormPassword(e.target.value)} required className="w-full px-4 py-2 border rounded-lg" />
            </div>

            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-2">
                <input id="registerMode" type="checkbox" checked={isRegisterMode} onChange={() => setIsRegisterMode(!isRegisterMode)} />
                <label htmlFor="registerMode" className="text-sm text-gray-600">Register new account</label>
              </div>

              <button type="submit" disabled={loadingAuth} className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:opacity-50">
                {loadingAuth ? 'Processing...' : isRegisterMode ? 'Register' : 'Sign In'}
              </button>
            </div>

            <div className="text-xs text-gray-500">
              By using email login you agree to the project auth endpoints. If these endpoints are not implemented on the server, this flow will fail; please implement server-side endpoints `/api/auth/register` and `/api/auth/login`.
            </div>
          </form>
        )}
      </div>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8">
          ...
          <AuthOptions redirectToLogin={redirectToLogin} navigate={navigate} />
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <SequenceSelector
              ...
              onSelectSequence={(sequence: Sequence) => setSelectedSequenceId(sequence.id)}
